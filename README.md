...
for inputfile in $(ls ../testdata/unrar.5.5.3.builds/unrar.x??.O?); do
  ...
  ./functionfingerprints ELF $inputfile 5 true >> /tmp/train/functions_$id.txt;
...
for inputfile in $(find ../testdata/unrar.5.5.3.builds/VS2015/ -iname \*.exe); do
  ...
  id=${shasum:0:16};
  filename=$(echo $inputfile | rev | cut -d"/" -f1 | rev);
  ./functionfingerprints PE $inputfile 5 true >> /tmp/train/functions_$id.txt;
  ...
  cat $inputfile.debugdump | grep Function | grep static | grep -v crt | while read -r line; do
    symbol=$(echo "$line" | cut -c78- | ./stemsymbol | tr -d '\n');
...
cat /tmp/train/attract.txt | cut -d" " -f1 | sort -R > /tmp/train/repulse_temp.txt;
cat /tmp/train/attract.txt | cut -d" " -f2 | sort -R > /tmp/train/repulse_temp2.txt;
paste -d" " /tmp/train/repulse_temp.txt /tmp/train/repulse_temp2.txt > /tmp/train/repulse.txt;

# Create functions.txt
cat /tmp/train/functions_* > /tmp/train/functions.txt

# Ok, we are done.

```

As a next step, we need to generate ```attract.txt```. 
